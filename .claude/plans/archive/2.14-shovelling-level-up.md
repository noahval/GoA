...
func _generate_upgrade_options(count: int) -> Array:
    ...
    if Level1Vars.technique_branch == "":
        ...
        for tech_id in TECHNIQUES.keys():
            ...
            var current_level = Level1Vars.selected_techniques.get(tech_id, {}).get("level", 0)
            var max_level = _get_max_level_for_technique(tech_id)
...
func _get_max_level_for_technique(technique_id: String) -> int:
    # All techniques max at level 5 (see 2.10-upgrade-pool.md)
    return TECHNIQUES[technique_id].get("max_level", 5)
```
...
func _on_upgrade_selected(tech_id: String):
    # Draw quality based on technique rarity (see 2.10-upgrade-pool.md#draw-quality-system)
    var draw_quality = _draw_quality_for_technique(tech_id)

    # Add technique with drawn quality
    if tech_id not in Level1Vars.selected_techniques:
        Level1Vars.selected_techniques[tech_id] = {
            "level": 1,
            "qualities": [draw_quality]
        }
    else:
        Level1Vars.selected_techniques[tech_id]["level"] += 1
        Level1Vars.selected_techniques[tech_id]["qualities"].append(draw_quality)
...
func _draw_quality_for_technique(tech_id: String) -> String:
    # Draw quality weights based on technique rarity
    # See 2.10-upgrade-pool.md#draw-quality-weights for full distribution
    ...
    match tech_rarity:
        "common":
            ...
            elif roll < 0.95: return "rare"
            elif roll < 0.99: return "epic"
            else: return "legendary"
        "rare":
            if roll < 0.50: return "common"
            elif roll < 0.85: return "rare"
            elif roll < 0.97: return "epic"
            else: return "legendary"
        "epic":
            if roll < 0.30: return "common"
            elif roll < 0.70: return "rare"
            elif roll < 0.95: return "epic"
            else: return "legendary"
        "legendary":
            if roll < 0.20: return "common"
            elif roll < 0.50: return "rare"
            elif roll < 0.85: return "epic"
            else: return "legendary"
        _:
            return "common"
```
...
func test_technique_level_increments():
    ...
    Level1Vars.selected_techniques["balance"] = {"level": 1, "qualities": ["common"]}
    Level1Vars.selected_techniques["balance"]["level"] += 1
    Level1Vars.selected_techniques["balance"]["qualities"].append("rare")

    assert_eq(Level1Vars.selected_techniques["balance"]["level"], 2, "Balance is level 2")
...
func test_technique_max_level():
    # All techniques max at level 5 (see 2.10-upgrade-pool.md)
    var max_level = 5  # Would call _get_max_level_for_technique("balance")
    assert_eq(max_level, 5, "All techniques max at 5")
```
...
### Max Levels
- **All techniques**: 5 levels (see [2.10-upgrade-pool.md](2.10-upgrade-pool.md))
