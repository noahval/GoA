# 1.20 Save System Cloud Storage Plan
...
Migration happens automatically when user switches authentication methods (device ID → username/Google). See plan [1.19-nakama-server.md](1.19-nakama-server.md) Part 5 for implementation details.
...
**Implementation**: See plan 1.19-nakama-server.md Part 5 and plan 1.21-auth-screen.md migration UI section.
...
### With Plan 1.19 (Nakama Server)
- Plan 1.19 provides `NakamaClient.save_game()` implementation
...
**Before This Plan**:
- [x] Nakama Server (plan 1.19) - Provides `save_game()` implementation
- [x] Global.gd stats system (plan 1.3)
- [x] Level1Vars autoload (plan 1.4)
- [x] Currency system (plans 1.5-1.7)

**After This Plan**:
- [ ] Auth Screen (plan 1.21) - Handles initial auth and migration UI
...
**Status**: References plan 1.19 for implementation, focuses only on save triggers
