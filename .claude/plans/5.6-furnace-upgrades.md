...
Add furnace upgrade variables to Level1Vars.gd:

```gdscript
# Furnace Material & Construction
var furnace_material_tier: int = 1  # 1=Cast Iron ... 7=Arc Furnace
var furnace_thickness_tier: int = 1  # 1=Thin ... 4=Reinforced
var furnace_lining_tier: int = 0  # 0=None ... 6=Silicon Carbide
var lining_durability: float = 0.0  # Hours remaining

# Special Systems
var steam_reservoir_level: int = 0
var steam_efficiency_level: int = 0
var has_cooling_system: bool = false
var has_forced_air: bool = false
var has_temp_monitoring: bool = false
```

### Phase 2: Max Heat Calculation

Implement temperature limit calculation:

```gdscript
func calculate_max_heat():
    var base_temps = {1: 700, 2: 900, 3: 1100, 4: 1550, 5: 1600, 6: 1800, 7: 3000}
    var thickness_mults = {1: 1.0, 2: 1.15, 3: 1.3, 4: 1.5}
    var lining_mults = {0: 1.0, 1: 1.3, 2: 1.6, 3: 1.9, 4: 2.2, 5: 2.5, 6: 2.5}

    var base = base_temps[furnace_material_tier]
    var calculated = base * thickness_mults[furnace_thickness_tier] * lining_mults[furnace_lining_tier]

    # Apply lining caps
    var lining_caps = {1: 960, 2: 1788, 3: 2072, 4: 2852, 5: 1650, 6: 1650}
    if furnace_lining_tier in lining_caps:
        calculated = min(calculated, lining_caps[furnace_lining_tier])

    return calculated
```

### Phase 3: Upgrade Purchase Logic

Create shop/upgrade dialog with tier progression and unlock requirements.

### Phase 4: Lining Degradation

Implement durability countdown and replacement system.

### Phase 5: Educational Popups

Show historical context on first purchase of each material/lining.

---

## Testing Scenarios

### Test 1: Material Progression
- Start with Cast Iron (700°C max)
- Purchase Wrought Iron (80 silver, requires 20h runtime)
- Verify max_heat = 900°C
- Purchase Mild Steel (250 silver, 50h runtime)
- Verify max_heat = 1,100°C
- Continue through all 7 tiers
- Verify final Arc Furnace reaches 3,000°C

### Test 2: Thickness Upgrades
- Purchase Standard Wall on Cast Iron
- Verify max_heat = 700 * 1.15 = 805°C
- Verify heat decay reduced by 10%
- Purchase Heavy Wall
- Verify max_heat = 700 * 1.3 = 910°C
- Verify decay reduced by 20%

### Test 3: Refractory Lining System
- Purchase Firebrick on Mild Steel
- Calculated: 1,100 * 1.0 * 1.3 = 1,430°C
- Capped at: 960°C (firebrick limit)
- Verify durability = 100 hours
- Run for 100 hours
- Verify lining needs replacement
- Purchase High-Alumina
- Calculated: 1,100 * 1.0 * 1.6 = 1,760°C
- Capped at: 1,788°C (no cap needed)
- Final: 1,760°C

### Test 4: Combined Upgrades
- Material: Cupola (1,550°C base)
- Thickness: Reinforced (1.5x)
- Lining: Mullite-Zirconia (1.9x)
- Calculated: 1,550 * 1.5 * 1.9 = 4,417°C
- Capped: 2,072°C (lining limit)
- Verify final = 2,072°C

### Test 5: Special System Upgrades
- Purchase Cooling System (-30% decay)
- Verify decay rate: 0.5 * 0.7 = 0.35/sec
- Purchase Forced Air (+25% heat from coal)
- Verify shovel generates: 1.0 * 1.25 = 1.25 heat/shovel
- Purchase Temperature Monitoring
- Verify exact temperature display (vs vague indicators)

### Test 6: Educational Popups
- Purchase each material tier for first time
- Verify historical flavor text appears
- Verify text stored in "Furnace Encyclopedia"
- Access encyclopedia from UI
- Verify all unlocked texts viewable

### Test 7: Unlock Requirements
- Try purchasing Cupola without Mild Steel
- Verify locked/error message
- Try purchasing Reinforced Wall at Tier 2
- Verify requires Tier 4+ material
- Verify runtime hour requirements enforced
