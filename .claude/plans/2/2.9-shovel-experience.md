...
**Location**: Coal delivery callback (around line 288-294)

**Current code** (from Plan 2.5):
```gdscript
...
**That's it.** One line of code for integration. The `if` check ensures XP is only awarded when coal is successfully consumed (returns `true`), preventing double-counting if the delivery zone triggers multiple times on the same coal.
