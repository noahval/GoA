...
- Delivery takes priority over dropped via position check (coal past furnace_line_x is never counted as dropped)

## Overview

This phase adds tracking for coal that successfully makes it into the furnace. A 15px wide detection zone is positioned inside the furnace opening. When coal enters, it's counted once, changes state to prevent dropped tracking, fades to red over 0.3 seconds, then despawns. The has_been_tracked flag combined with a position-based check in the border handler ensures delivery always takes priority over drop tracking, even for fast-moving coal where both zones trigger in the same physics frame.
...
func _on_coal_entered_delivery_zone(body: Node2D):
    ...
    # Only process if coal was actually consumed (not already tracked)
    if body._on_entered_delivery_zone():
        # Future: Plan 2.9 awards XP here
        pass
```
...
**Notes:**
- Guard clause prevents double-counting if delivery zone triggers twice (matches drop handler pattern)
- Returns true on successful delivery, false if already tracked (used by Plan 2.9 for XP awards)
...
- [ ] Fast-moving coal past furnace_line_x hitting right border → not counted as dropped (position check)
...
**Coal being counted twice (delivered AND dropped):**
...
- Fast-moving coal fix: `_on_coal_entered_border()` checks `body.global_position.x >= furnace_line_x` and skips drop counting for coal past furnace line
...
**Why position-based furnace check in border handler?**
- Fast-moving coal can enter both delivery zone AND right border zone in same physics frame
- Godot doesn't guarantee signal firing order between multiple Area2D zones
- If border signal fires before delivery signal, coal would be counted as dropped incorrectly
- Position check (`body.global_position.x >= furnace_line_x`) in `_on_coal_entered_border()` ensures coal past furnace line is never counted as dropped
- This is more reliable than depending on `has_been_tracked` flag alone, which requires delivery handler to run first
- See 2.x-dropped-coal.md for the border handler implementation
