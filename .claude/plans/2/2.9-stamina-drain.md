...
func update_stamina_drain(delta: float):
	...
	if should_drain:
		...
		# Apply drain (drain_rate is per second, multiply by delta for this frame)
		var drain_amount = drain_rate * delta
		Level1Vars.modify_stamina(-int(ceil(drain_amount)))
...
- Multiply by `delta` to get drain for this frame
- Use `ceil()` to round up (prevents micro-drains from being ignored)
...
**Cause:** Using `int()` truncation instead of `ceil()` for drain amount
- Small drain values (0.3 stamina/sec at 60fps = 0.005 per frame) get truncated to 0
- Drain only happens every few frames, feels laggy

**Solution:** Use `ceil(drain_amount)` to ensure minimum 1 stamina drain per frame when active
- Alternative: Accumulate fractional drain in a variable, apply whole numbers only
...
**Solution:** Recalculate boundary when playarea size changes
- Connect to ResponsiveLayout resize signal (if available)
- Or recalculate every frame (slightly less efficient but robust):
```gdscript
func _physics_process(delta):
	# Recalculate boundary (handles resize)
	if playarea:
		var playarea_rect = playarea.get_global_rect()
		work_zone_boundary_x = playarea_rect.position.x + (playarea_rect.size.x / 3.0)
```
...
- [ ] Work zone boundary calculation in shovel.gd
...
- [ ] Drain calculation uses `ceil()` to prevent rounding issues
