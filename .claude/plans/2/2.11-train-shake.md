...
```gdscript
# Train shake mechanic
var shake_interval_min: float = 20.0  # Minimum seconds between shakes
...
var shake_coal_impulse_strength: float = 150.0  # Physics impulse applied to coal (pixels/sec)
...
func apply_coal_shake():
    if not coal_container:
        return

    # Get all coal RigidBody2D children
    for child in coal_container.get_children():
...
- **Shake interval**: Random 20 to 60 seconds - Prevents pattern memorization
...
- **Coal impulse strength**: 150.0 pixels/sec - Applied ONCE at shake start
...
- Coal impulse at 150 applied once = enough to make coal jump without flying off screen
...
**Solution**: Impulse is applied ONCE at shake start (not per-frame), so strength of 150 should be safe. If coal still flies off, reduce `shake_coal_impulse_strength` in Level1Vars to 100 or 75. Higher values = more chaos but risk losing coal permanently.

**Note**: Original plan had per-frame impulses which would accumulate to ~9000 pixels/sec - this was corrected to single impulse.
...
- Interval: 30-45 seconds average (reasonable frequency without being oppressive)
- Warning: 500ms (challenging but fair for skilled players)
- Big shake: 1.0 second average (enough time to lose coal but not forever)
- Impulse: 150 (coal moves but doesn't instantly fly away)
