...
- Coal tap spawns coal at 6/second until 100 coal exist in play area
...
This phase implements the coal pieces and automatic coal tap spawning system. Coal pieces are physics objects that interact with the shovel through friction and damping - no manual attachment logic. Coal spawns continuously from a tap position at 6 pieces per second, falling into a container in the bottom-left of the play area, until 100 total coal pieces exist.
...
**CRITICAL - Godot Collision Layer Bitmask Values:**
- Layer 1 = bit 0 = 2^0 = value 1
- Layer 2 = bit 1 = 2^1 = value 2
- Layer 3 = bit 2 = 2^2 = value 4
- Layer 4 = bit 3 = 2^3 = value 8
- etc.

**To set an object on Layer 3, use `collision_layer = 4`, NOT 3!**

**Layer Setup** (extends 2.x-physics-objects-setup.md):
- Layer 1: World objects (furnace walls, container walls) - bitmask value 1
- Layer 2: Shovel - bitmask value 2
- Layer 3: Coal pieces (NEW - added in this phase) - bitmask value 4

**Collision Configuration**:
- Container walls: collision_layer = 1 (Layer 1) - acts as world collision
- Shovel: collision_layer = 2 (Layer 2), collision_mask = 5 (binary 101 = layers 1 and 3) - collides with world and coal
- Coal: collision_layer = 4 (Layer 3), collision_mask = 7 (binary 111 = layers 1, 2, 3) - collides with world, shovel, and other coal

---

## Implementation Tasks

### 1. Create Coal Container with Walls (Sloped + Off-Screen)

Create a container in the bottom-left of the play area (25% width x 15% height) with collision walls on bottom, left, and right sides. The container extends off-screen to the left with a sloped bottom so coal rolls in naturally.
...
**Container Configuration Constants (in furnace.gd):**
```gdscript
const WALL_PHYSICS_EXTENSION: float = 30.0  # Invisible physics padding beyond visible walls
const CONTAINER_SLOPE_HEIGHT: float = 25.0  # How much higher left side is than right
var offscreen_extension = 60.0  # How far left the container extends off-screen
```

**In furnace.gd setup_physics_objects():**
- Position container at bottom-left, extended off-screen: `Vector2(-offscreen_extension, playarea_size.y - container_height)`
- Calculate container dimensions: 25% width + offscreen_extension, 15% height of play area
- Calculate left wall height: 30% of play area height (2x container height)
- **SLOPED BOTTOM**: Left side is `CONTAINER_SLOPE_HEIGHT` (25px) higher than right side
  - `slope_left_y = container_height - CONTAINER_SLOPE_HEIGHT`
  - `slope_right_y = container_height`
  - Bottom collision is rotated rectangle matching slope angle
- **EXTENDED PHYSICS WALLS**: All walls extend beyond visible edges by `WALL_PHYSICS_EXTENSION` (30px) to prevent tunneling
- Wall thickness: 5px (visual), 35px (physics with extension)
- **LEFT WALL**: Off-screen, visual line hidden (empty PackedVector2Array)
- **BOTTOM WALL**: Visible portion only draws from `offscreen_extension` to `total_container_width`
- **RIGHT WALL**: Physics extends 30px further right

```gdscript
# Slope calculations
var slope_dx = total_container_width
var slope_dy = slope_right_y - slope_left_y
var slope_length = sqrt(slope_dx * slope_dx + slope_dy * slope_dy)
var slope_angle = atan2(slope_dy, slope_dx)

# Bottom wall - rotated rectangle for slope
...
**Why sloped bottom?** Coal rolls naturally from off-screen spawn point into the visible container area. Creates more natural "coal feeding in" visual.

**Why off-screen extension?** Coal spawns where player can't see it, then rolls into view. Eliminates "appearing from nowhere" visual.

**Why extended physics walls?** Prevents coal from tunneling through walls when shovel pushes with force. The 30px invisible extension catches fast-moving coal.
...
func _ready():
	...
	# Collision layers
	collision_layer = 4  # Layer 3 (2^2 = 4, NOT 3!)
...
Continuously spawn coal from a tap position into the container at 6 pieces per second until 100 total coal exist.
...
**Note:** The tap position is now in the off-screen portion of the container. Coal spawns where the player can't see it, then rolls down the slope into the visible area.
...
2. Every 0.167 seconds (6/second), spawn single coal piece
...
**Design Values**:
- Spawn rate: 6 per second (0.167s interval)
...
- [ ] collision_layer = 4 (Layer 3), collision_mask = 7
...
- [ ] Coal spawns every 0.167 seconds (6/second)
...
3. Expected: Coal spawns from tap at 6 pieces per second
...
- Verify shovel collision_mask includes layer 3 (should include bit value 4)
- Verify coal collision_layer = 4 (Layer 3 = 2^2 = 4)
...
- COAL_SPAWN_RATE: 0.167s (6/sec) → slower: 0.25s (4/sec), faster: 0.125s (8/sec)
...
- With 6/second spawn rate, fills in ~17 seconds
