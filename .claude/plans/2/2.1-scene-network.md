...
[CLICK HERE TO OPEN MERMAID LIVE EDITOR](https://mermaid.live/)
...
```mermaid
graph TB
    %% See 2.1-scene-network.mmd for the full editable diagram
    %% This is just a preview - edit the .mmd file instead

    furnace["<b>Furnace</b><br/>Actions: Shovel Coal, Auto-Shovel<br/>Forward: To Bar"]
    bar["<b>Bar</b><br/>Forward: Carriage, Dorm, Secret<br/>Back: To Furnace"]
    shop["<b>Shop</b><br/>Back: auto-returns"]
    dream["<b>Dream</b><br/>Back: auto-returns"]
    carriage["<b>Coppersmith Carriage</b><br/>Forward: ATM, Office<br/>Back: To Bar"]
    atm["<b>ATM</b><br/>Back: To Carriage"]
    dorm["<b>Dormitory</b><br/>Back: To Bar"]
    secret["<b>Secret Passage</b><br/>Forward: Puzzle<br/>Back: To Bar"]
    puzzle["<b>Secret Puzzle</b><br/>Back: To Passage"]
    office["<b>Overseer Office</b><br/>Back: To Carriage"]

    furnace ==>|Forward Nav| bar
    furnace ==>|Forward Nav| shop
    furnace ==>|Forward Nav| dream
    bar ==>|Forward Nav| carriage
    bar ==>|Forward Nav| dorm
    bar ==>|Forward Nav| secret
    carriage ==>|Forward Nav| atm
    carriage ==>|Forward Nav| office
    secret ==>|Forward Nav| puzzle

    bar -.->|Back Nav| furnace
    shop -.->|Back Nav| furnace
    dream -.->|Back Nav| furnace
    carriage -.->|Back Nav| bar
    dorm -.->|Back Nav| bar
    secret -.->|Back Nav| bar
    atm -.->|Back Nav| carriage
    office -.->|Back Nav| carriage
    puzzle -.->|Back Nav| secret

    classDef copperScene fill:#5c2e00,stroke:#9a6229,stroke-width:2px,color:#fff
    class furnace,bar,shop,dream,carriage,atm,dorm,secret,puzzle,office copperScene
```

### Diagram Legend

- **Solid thick arrows** (`==>`) = Forward Navigation (light blue buttons, 2px border)
- **Dotted arrows** (`-.->`) = Back Navigation (navy blue buttons, 1px border)
- **Node contents**:
  - Scene name (bold at top)
  - Actions: Gameplay buttons (orange, top of menu)
  - Forward Nav: Advancement buttons (light blue, middle)
  - Back Nav: Return buttons (navy, bottom)
  - Settings button always at bottom (not shown in diagram)

### Quick Reference for Editing

**Add a scene node** (in [2.1-scene-network.mmd](2.1-scene-network.mmd)):
```mermaid
newscene["<b>New Scene</b><br/>---<br/>Actions:<br/>- Action 1<br/>Forward Nav:<br/>- To Scene X<br/>Back Nav:<br/>- To Scene Y"]
```

**Add forward navigation arrow**:
```mermaid
sceneA ==>|Forward Nav| sceneB
```

**Add back navigation arrow**:
```mermaid
sceneB -.->|Back Nav| sceneA
```
