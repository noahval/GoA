...
graph TB
    ...
    dream["<b>Dream</b><br/>---<br/>Actions:<br/>- (rest)"]
...
3. Add scene-specific content to `MainLayout/Menu` and `MainLayout/PlayArea`
...
├── Background (TextureRect) - Auto-loaded by ResponsiveLayout
...
**Note**: Action buttons will be added later by feature-specific plans at the top of the Menu VBoxContainer.
...
- Text format: "Back to [Scene Name]"
...
4. Button node naming convention:
   - Forward: `GoTo[SceneName]Button` (e.g., `GoToBarButton`)
   - Back: `BackTo[SceneName]Button` (e.g., `BackToFurnaceButton`)

**Example from current .mmd file:**
```
furnace --> n1["Mind"]
```
- **Furnace** gets Forward Nav button: `GoToMindButton` with text "Go to Mind"
- **Mind** gets Back Nav button: `BackToFurnaceButton` with text "Back to Furnace"

```
dorm --> bar & carriage
```
- **Dorm** gets two Forward Nav buttons:
  - `GoToBarButton` with text "Go to Bar"
  - `GoToCarriageButton` with text "Go to Coppersmith Carriage"
- **Bar** gets Back Nav button: `BackToDormButton` with text "Back to Dormitory"
- **Carriage** gets Back Nav button: `BackToDormButton` with text "Back to Dormitory"

**Implementation:**
- Add Button nodes to `MainLayout/Menu/` in each scene
...
func connect_navigation():
    ...
    $MainLayout/Menu/GoToBarButton.pressed.connect(func(): navigate_to("bar"))
    $MainLayout/Menu/GoToShopButton.pressed.connect(func(): navigate_to("shop"))
    $MainLayout/Menu/GoToDreamButton.pressed.connect(func(): navigate_to("dream"))
    $MainLayout/Menu/BackToFurnaceButton.pressed.connect(func(): navigate_to("furnace"))
...
- [ ] Navigation button text follows format ("Go to X", "Back to X")
...
**Plan Version**: 2.1.1 (Simplified - removed action buttons, unlock conditions, era metadata; strict validation only; .mmd as visual source of truth)
