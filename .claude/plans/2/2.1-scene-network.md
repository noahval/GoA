...
- **Node contents**:
  ...
  - **Currency Display Indicator** (line after title):
    - `c` = Add copper currency display component to this scene's Menu (see [1.27-currency-display.md](../1/1.27-currency-display.md))
    - Position: Top of Menu VBoxContainer, before action buttons
    - Component: `CurrencyDisplay` with `currency_type = "copper"`
    - Auto-updates via `Global.refresh_all_currency_displays()` (see [1.30-currency-manager.md](../1/1.30-currency-manager.md))
...
If the scene has a `c` indicator in [2.1-scene-network.mmd](2.1-scene-network.mmd) (line after scene title), add a CurrencyDisplay component:

1. Instance `res://ui/currency_display.tscn` (from plan [1.27-currency-display.md](../1/1.27-currency-display.md))
2. Add as first child of `mainarea/Menu` (top position)
3. Set `currency_type` to "copper"
4. Component auto-updates via `Global.refresh_all_currency_displays()` (from [1.30-currency-manager.md](../1/1.30-currency-manager.md))

**Example scenes with currency display:**
- Dorm (has `c` indicator in .mmd file)
...
- [ ] Currency displays added to scenes with "c" indicator (e.g., Dorm)
  - [ ] CurrencyDisplay component positioned at top of Menu VBoxContainer
  - [ ] `currency_type` set to "copper"
  - [ ] Component from `res://ui/currency_display.tscn` ([1.27-currency-display.md](../1/1.27-currency-display.md))
