...
func _ready():
	...
	# CRITICAL: Defer work zone boundary calculation until after layout is applied
	# ResponsiveLayout needs one frame to position the playarea
	# Without await, playarea.get_global_rect() returns incorrect values
...
**Why await before calculating boundary?**
- ResponsiveLayout.apply_to_scene() in furnace.gd positions the playarea
- Layout calculations happen during the first process frame
- Without await, playarea.get_global_rect() returns pre-layout values (often negative positions)
- This causes work_zone_boundary_x to be incorrect, making stamina drain everywhere
- See plan [2.3-physics-objects.md](2.3-physics-objects.md) for more on ResponsiveLayout timing
...
- Drain rate calculated directly each frame (simple math, not a performance issue)
- No caching, no debounce, no complexity
- Multiply drain rate by `delta` to get fractional drain for this frame
- `modify_stamina()` clamps to [0, max] and emits signals
- Early exit when no coal prevents unnecessary calculation
...
Add debug toggle and visualization (optional):
```gdscript
...
- Drain only occurs when coal_bodies_on_shovel.size() >= 1 AND in_work_zone
- Drain rate = base (0.2) + (coal_count × 0.1) stamina per second, calculated each frame
- Stamina bar updates smoothly with direct float subtraction
- modify_stamina() clamps to [0, max]
- Debug overlay toggles with F3 key and shows current drain rate (manual, not automatic)
...
- [shovel.gd](../../game/v0.1/level1/shovel.gd) - Add coal counting, work zone detection, stamina drain calculation, optional debug visualization
- [level_1_vars.gd](../../game/v0.1/level1/level_1_vars.gd) - Change stamina to float, add drain rate variables
- **Note**: Shovel coal detection shape uses constants (COAL_DETECTION_WIDTH, COAL_DETECTION_HEIGHT) defined in shovel.gd (matches shovel width from plan 2.3)
...
- Godot automatically fires `body_exited` when coal is freed from scene tree
- Array size is calculated on-demand, so freed coal automatically stops being counted
...
**Solution:** Already handled in this plan:
```gdscript
if coal_bodies_on_shovel.size() == 0:
	return  # Early exit
```
...
- [ ] Coal tracking variable added to shovel.gd (coal_bodies_on_shovel array only)
...
- [ ] `update_stamina_drain()` function implemented with early exit and direct calculation
- [ ] Drain calculation uses direct float subtraction (no accumulator, no cache)
- [ ] (Optional) Debug visualization in shovel.gd with F3 toggle using draw_string()
