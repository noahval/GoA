...
- Bars follow theme design guidelines from [1.8-default-theme.md](../1/1.8-default-theme.md)
...
- Use ProgressBar template from [1.8-default-theme.md](../1/1.8-default-theme.md#progressbar) (lines 316-333)
- Use named progress bars from [1.8-default-theme.md](../1/1.8-default-theme.md#progressbar-variables):
...
Each bar has a centered text label to identify it:
...
For StaminaBar and FocusBar, labels are defined in `furnace.tscn` as child Label nodes with:
- `anchors_preset = 15` (full rect)
- `horizontal_alignment = 1` (center)
- `vertical_alignment = 1` (center)
- `theme_override_font_sizes/font_size = 14` (smaller than 25px base for 20px bar)

For XPBar, the label is created programmatically in `setup_xp_bar()`:
```gdscript
# Add to menu FIRST so anchors work correctly
menu.add_child(xp_bar)
menu.move_child(xp_bar, 2)

# Add label AFTER bar is in scene tree (anchors need parent size)
...
**IMPORTANT gotchas for programmatic labels:**
1. Label must be added AFTER bar is in scene tree (anchors need parent sizing)
2. Use `set_anchors_and_offsets_preset()` NOT `set_anchors_preset()` - the latter only sets anchors without resetting offsets, causing misalignment
3. Set `font_size` override BEFORE `ResponsiveLayout.apply_to_scene()` runs, or ResponsiveLayout will skip it (it checks for existing overrides)

**Label styling (per [1.8-default-theme.md](../1/1.8-default-theme.md#progressbar)):**
- Font size: 14px (compact for 20px bars)
- Anchors: PRESET_FULL_RECT (fill bar area)
...
**ALL styling values are defined in [1.8-default-theme.md](../1/1.8-default-theme.md#progressbar).**
...
- **Template structure**: Lines 316-333 in [1.8-default-theme.md](../1/1.8-default-theme.md)
...
|                                        |
| [#####  Stamina  #####----------]      | <- Green bar with centered label
| [####   Focus    ######---------]      | <- Blue bar with centered label
| [##      XP      ##-------------]      | <- Dark orange bar with centered label
|                                        |
...
**Decision 2:** Use existing named progress bars from [1.8-default-theme.md](../1/1.8-default-theme.md)
- **Rationale**:
...
- **Rationale**:
  - Progress bar template and named bars already defined in [1.8-default-theme.md](../1/1.8-default-theme.md)
...
**Open Questions:**
- [x] Should bars have labels showing numeric values? -> Implemented with simple text labels ("Stamina", "Focus", "XP")
...
**Scene setup:**
- [x] StaminaBar ProgressBar node exists in furnace.tscn Menu area
- [x] FocusBar ProgressBar node exists in furnace.tscn Menu area
- [x] XPBar created programmatically via setup_xp_bar()
- [x] StaminaBar has child Label with text "Stamina"
- [x] FocusBar has child Label with text "Focus"
- [x] XPBar has child Label with text "XP" (created programmatically)
...
**Status**: Implemented - bar labels added ("Stamina", "Focus", "XP")
