...
### Job Board Integration (See 1.x-job-board.md)

This plan covers the **Overtime upgrade** which extends the offline cap duration. The **Job Board** (separate plan) handles:
- Job selection UI where player assigns protagonist to idle work
- Multiple job types with active/passive modes (dishwashing, etc.)
- Idle efficiency progression: starts at 0.1x active earnings, improves up to 0.5x with mastery
- Eventually: hireable workers who can each idle one job

The Overtime system here controls **how long** idle earnings accumulate; the Job Board controls **what job** and **how efficiently**.
...
- Note: This is separate from per-job idle efficiency (Job Board system)
