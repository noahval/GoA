...
This plan provides the **core offline earnings infrastructure** - timestamp verification and duration caps. It does NOT define job-specific earnings rates or efficiency multipliers. Those belong to the Job Board (2.x-job-board.md) which owns job definitions and mastery progression.
...
- **2.x-job-board.md**: Provides `JobBoardManager` which handles job assignment, mastery tiers, efficiency multipliers, and `calculate_idle_earnings()`
- **1.x-nakama-server.md**: Provides server timestamp verification to prevent clock manipulation

**Note**: This plan provides the groundwork (OfflineEarningsManager singleton) that 2.x-job-board.md builds upon. Job Board implements `JobBoardManager.calculate_idle_earnings()` which this plan calls.
...
2. Calling `JobBoardManager.calculate_idle_earnings()`
...
# result contains: {earnings: int, currency: String, job_name: String, efficiency: float}
```
...
func _show_notification(elapsed: int, capped: int, result: Dictionary):
    ...
    if result.is_empty():
        ...
        message += "Your idle work (%s at %.1fx):\n" % [result.job_name, result.efficiency]
        message += "- Earned: %d %s\n\n" % [result.earnings, result.currency]
        if exceeded:
            message += "You hit your offline cap!"
        else:
            message += "Tip: Improve mastery by playing actively!"
...
**Why no efficiency penalty here?**
- Each job defines its own idle rate
- Dishwashing plan handles dishwash idle efficiency
- Keeps this plan focused on cap/timestamp logic

### Separation of Concerns

| This Plan (2.17 Offline Earnings) | Job Board Plan (2.x-job-board.md) |
|-----------------------------------|-----------------------------------|
| Server timestamp verification | Job assignment (`current_idle_job`) |
| Duration cap (2 hours) | Mastery tier tracking |
| OfflineEarningsManager singleton | Idle efficiency calculation |
| Welcome-back notification UI | `calculate_idle_earnings()` math |
| Fallback to local time | Currency type per job |

**Call Flow**: Game Load → OfflineEarningsManager → JobBoardManager → Currency awarded
