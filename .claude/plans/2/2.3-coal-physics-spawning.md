...
- Coal tap spawns coal at 2/second until 100 coal exist in play area
...
This phase implements the coal pieces and automatic coal tap spawning system. Coal pieces are physics objects that interact with the shovel through friction and damping - no manual attachment logic. Coal spawns continuously from a tap position at 2 pieces per second, falling into a container in the bottom-left of the play area, until 100 total coal pieces exist.
...
2. Every 0.5 seconds (2/second), spawn single coal piece
...
6. Cleanup reduces count, allowing spawning to resume

**Design Values**:
- Spawn rate: 2 per second (0.5s interval)
- Tap position: 15% into container width from left, 10px above container top
- Max coal: 100 pieces (performance cap)
- Coal cleanup: 2000px from origin OR 30s lifetime (handled in coal_piece.gd)
...
- [ ] Coal spawns every 0.5 seconds (2/second)
...
3. Expected: Coal spawns from tap at 2 pieces per second
...
3. Throw some coal offscreen (move with shovel)
4. Wait for cleanup (2000px distance or 30s lifetime)
5. Expected: Count drops below 100, spawning resumes
...
- COAL_SPAWN_RATE: 0.5s (2/sec) → slower: 1.0s (1/sec), faster: 0.33s (3/sec)
...
**Why 2 coal per second spawn rate?**
- Fast enough to keep container supplied
- Not so fast that it overwhelms physics or creates lag
- Reaches 100 coal cap in 50 seconds (reasonable time to fill)
...
- With 2/second spawn rate, fills in 50 seconds
