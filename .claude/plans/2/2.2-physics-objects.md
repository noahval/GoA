...
**Goal**: Create the physical environment for shovelling gameplay - furnace opening with obstacles and shovel surface.
...
### 1. Furnace Opening with Physical Obstacles
...
## Coal Container Physics Extensions (Added Post-Implementation)

The coal container was enhanced with sloped bottom and extended physics walls to improve gameplay:

### Sloped Bottom
The container bottom is no longer flat - it slopes downward from left to right:
- Left side is `CONTAINER_SLOPE_HEIGHT` (25px) higher than right side
- Coal naturally rolls toward the visible play area
- Creates more natural "coal rolling in" visual

### Off-Screen Extension
The container extends 60px off-screen to the left:
- Coal spawns in the off-screen portion
- Coal rolls into view naturally (no "appearing from nowhere")
- Left wall visual is hidden (off-screen)

### Extended Physics Walls (Anti-Tunneling)
Physics colliders extend beyond visible walls by `WALL_PHYSICS_EXTENSION` (30px):
- **Left wall**: Extends 30px further left (35px total width)
- **Bottom wall**: Extends 30px downward below visible slope
- **Right wall**: Simple 15px wide wall with no extension (see below)

This prevents coal from tunneling through walls when the shovel pushes with force.

### Right Wall Design
The right wall is a 15px wide ConvexPolygonShape2D with a slanted top:
- **Width**: 15px (unified collision and visual)
- **Height**: 22% of playarea height (`right_wall_height_percent`)
- **Slanted top**: Left-top corner is 10px lower than right-top (`right_wall_top_slant`)
- **Visual**: Solid black filled Polygon2D (not just an outline)

The slanted top causes coal that lands on top of the wall to roll back into the container instead of balancing on the edge.

### Constants (in furnace.gd)
```gdscript
const WALL_PHYSICS_EXTENSION: float = 30.0  # Invisible physics padding (left/bottom walls)
const CONTAINER_SLOPE_HEIGHT: float = 25.0  # Slope drop from left to right
var offscreen_extension = 60.0  # How far left container extends off-screen
var right_wall_height_percent: float = 0.22  # 22% of playarea height
var right_wall_width = 15.0  # Simple 15px wall
var right_wall_top_slant = 10.0  # Slant for coal rollback
```
