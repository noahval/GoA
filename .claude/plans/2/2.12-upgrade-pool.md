...
**IMPORTANT**: GDScript extends Node pattern for preloading.
...
# Referenced by Mind scene (2.x-shovelling-level-up.md) and effect application (this plan)
extends Node
...
func get_save_data() -> Dictionary:
    return {
        ...
        "nodrop_combo_unlocked": nodrop_combo_unlocked,
        "heavy_combo_unlocked": heavy_combo_unlocked,
        "show_exact_technique_values": show_exact_technique_values,
        # Save combo state to prevent save-scumming
...
func load_save_data(data: Dictionary):
    ...
    nodrop_combo_unlocked = data.get("nodrop_combo_unlocked", false)
    heavy_combo_unlocked = data.get("heavy_combo_unlocked", false)
    show_exact_technique_values = data.get("show_exact_technique_values", true)
    # Load combo state (prevents save-scumming by preserving combo breaks across reloads)
...
**Save-Scumming Prevention**: Combo state persists across save/load. Additionally, the game automatically saves when combo breaks occur (see Part 7), preventing players from reloading to restore lost combos.
...
# Connect to combo signal for dynamic updates
...
func _on_entered_drop_zone():
    ...
    if Level1Vars.nodrop_combo_unlocked:
        ...
        if Level1Vars.forgiveness_charges > 0:
            ...
            # Auto-save to prevent save-scumming
            Level1Vars.save_run()
...
```gdscript
    ...
    if Level1Vars.heavy_combo_unlocked:
        ...
        Level1Vars.recent_delivery_timestamps.clear()
        Level1Vars.emit_signal("heavy_combo_changed", 0, 0.0)
        # Auto-save already triggered above for no-drop combo, so no need to save again here
```
...
- [ ] Save/load preserves technique selections AND combo state (prevents save-scumming)
- [ ] Auto-save triggers when no-drop combo breaks
- [ ] Auto-save triggers when heavy combo timer expires
...
- `game/v0.1/level1/mind.gd` - Mind scene script for upgrade selection UI (if doesn't exist)
...
- [ ] Implement reset_techniques() with full state reset
- [ ] Update save/load functions to persist combo state (prevents save-scumming)
- [ ] Add auto-save triggers on combo breaks (no-drop and heavy timer expiration)
- [ ] Test save/load preserves techniques AND combo state correctly
...
**Decision**: Techniques and unlock flags persist across runs (saved). Combo counters and timers reset each run.

**Rationale**: Permanent progression (techniques) gives meta-game depth. Fresh combo state each run maintains challenge and prevents degenerate strategies.
...
**Decision**: Combo state persists across save/load, and game auto-saves when combos break.

**How it works**:
1. All combo counters, timers, and charges are saved to disk
2. When no-drop combo breaks (coal dropped without forgiveness), auto-save triggers
3. When heavy combo timer expires, auto-save triggers
4. Player cannot reload to restore lost combo progress

**Rationale**: Preserves the high-stakes nature of combo systems. Forgiveness mechanic provides intentional safety net, reload exploits would undermine that design.

**Alternative considered**: Don't save combo state, reset on load - rejected because it punishes legitimate mid-run saves and creates confusion about what persists.
...
**Version**: 5.4 (Uniform quality distribution + dual-color cards + save-scumming prevention)
...
- **SAVE SYSTEM**: Combo state now persists across save/load to prevent save-scumming
- Added auto-save triggers when no-drop combo breaks
- Added auto-save triggers when heavy combo timer expires
- Updated save/load functions to include all combo state variables
- Added Design Notes sections: Uniform Quality Distribution, Dual-Color Card System, Save-Scumming Prevention
