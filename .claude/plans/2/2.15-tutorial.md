...
**Important**:
- The `await get_tree().process_frame` is **required** - calling `change_scene_to_file()` directly in `_ready()` triggers "Parent node is busy adding/removing children" warnings because Godot is still constructing the scene tree.
- The `return` statement prevents the rest of `_ready()` from executing after the scene change.
...
**Decision 4**: Use `await get_tree().process_frame` before scene changes in `_ready()`
- **Rationale**: Calling `change_scene_to_file()` directly during `_ready()` causes "Parent node is busy adding/removing children" warnings because Godot is still constructing the node tree
- **Solution**: Wait one frame for tree construction to complete before changing scenes
- **Pattern**: Always use `await get_tree().process_frame` before `change_scene_to_file()` when inside `_ready()`
