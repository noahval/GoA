...
Menu/ (existing VBoxContainer at AspectContainer/MainContainer/mainarea/Menu)
  ...
  ComboContainer (PanelContainer, index 2) - NEW, single-line layout
    Properties: visible = false (initially), dark purple bg at 30% opacity
    HBoxContainer (spacing = 40, alignment = center)
      CleanStreakLabel (Label - "No-drop: 0/20", font_size = 14)
      ForgivenessLabel (Label - "[0]", font_size = 14, visible = false)
      HeavyStacksLabel (Label - "Heavy: 0", font_size = 14)
      HeavyTimerBar (ProgressBar - 50x12, shows remaining time)
  BenefitsTooltip (PanelContainer, index 3) - NEW, hidden by default
    Properties: visible = false, position anchored to right of combo panel
...
**Layout Design**: Single-line display with smaller font (14px), centered, with wide spacing (40px). Individual elements show/hide based on unlock state:
- CleanStreakLabel: visible when clean_streak_unlocked (displays "No-drop: X/Y")
- ForgivenessLabel: visible when forgiveness_max_capacity > 0
- HeavyStacksLabel: visible when heavy_combo_unlocked
- HeavyTimerBar: visible when heavy unlocked AND timer > 0
...
**How to apply**: Create StyleBoxFlat for combo panel and tooltip in Godot editor or via code.

**StyleBox Properties** (dark purple, 30% opacity - matches theme guidelines):
- Background: `Color(0.2, 0.1, 0.25, 0.3)` - dark purple, 30% opacity
- Border: `Color(0.35, 0.2, 0.4, 0.4)` - subtle purple
- Border width: 1px (all sides)
- Corner radius: 2px (all corners)
- Content margin: 6px horizontal, 4px vertical

**Color Rationale**: Dark purple chosen to:
- Distinguish from orange (standard/negative) and blue (navigation/affirmative)
- Provide a "status/buff" feel appropriate for combo display
- Maintain 30% opacity per theme guidelines (1.8-default-theme.md)
...
### Full Menu Layout - Both Systems Unlocked (Single Line, Centered)

```
+----------------------------------------+
| [Stamina Bar ######################]   |
| [Focus Bar   ######################]   |
|    No-drop: 15/30      Heavy: 3 [===]  |  <- Centered, wide spacing (40px)
+----------------------------------------+
| [To Mind] [DEBUG...]                   |
+----------------------------------------+
```

### Only Clean Streak Unlocked

```
+----------------------------------------+
| [Stamina Bar ######################]   |
| [Focus Bar   ######################]   |
|           No-drop: 15/30               |  <- Centered
+----------------------------------------+
| [To Mind] [DEBUG...]                   |
+----------------------------------------+
```

### Only Heavy Combo Unlocked
...
### Initial State (No Combos Unlocked)
...
| [Focus Bar   ######################]   |
+----------------------------------------+
| [To Mind]                              |  <- Combo line hidden entirely
+----------------------------------------+
```
...
**Resolved Decisions:**
- [x] Forgiveness charges shown in clean streak panel: "No-drop: 15/30 [2]"
...
- [x] Single-line layout, centered, with wide spacing (40px) - no separator
- [x] Dark purple background Color(0.2, 0.1, 0.25, 0.3) - distinct from orange/blue, 30% opacity per theme
- [x] Font size 14px to match progress bar labels (smaller, less intrusive)
- [x] Renamed "Streak" to "No-drop" for clarity
...
**Changelog v1.5** (2026-01-05):
- **LAYOUT REFINEMENT**: Centered elements with wider spacing
  - HBoxContainer alignment set to center (alignment = 1)
  - Spacing increased from 8px to 40px for cleaner separation
  - Removed SeparatorLabel ("|") - whitespace separation looks cleaner
- **NAMING**: Renamed "Streak" to "No-drop" for clarity
  - Display now shows "No-drop: 0/20" instead of "Streak: 0/20"

**Changelog v1.4** (2026-01-05):
- **LAYOUT CHANGE**: Combined streak and heavy into single-line display
  - Changed from two stacked PanelContainers to single PanelContainer with HBoxContainer
  - Font size reduced to 14px to match progress bar labels
- **COLOR CHANGE**: Dark purple background instead of dark grey
  - New: `Color(0.2, 0.1, 0.25, 0.3)` - dark purple at 30% opacity
  - Old: `Color(0.1, 0.1, 0.1, 0.7)` - dark grey at 70% opacity
  - Matches theme guidelines (30% opacity) and avoids blue/orange reserved colors
- **STRUCTURE SIMPLIFICATION**:
  - Removed NodropComboPanel and HeavyComboPanel wrapper nodes
  - ComboContainer is now the single PanelContainer
  - Simplified hover logic (one panel instead of two)
- Updated all @onready paths in furnace.gd
- Updated visibility logic to control individual labels + separator
