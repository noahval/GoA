...
- Requires 2.6-delivered-coal.md for coal_delivered tracking
- Requires notification system for overseer dialog
- Requires vignette overlay system for red flash
...
- Constitution gain from whipping creates subtle long-term benefit (toughening up)
...
- **Per whip**: +0.5 constitution (subtle, accumulates over time)
- **Max constitution**: Capped at some reasonable value (TBD based on constitution system)
...
### 2. Global Vars - Constitution

Add or verify constitution variable in Global autoload.

**In global.gd:**
```gdscript
# Player constitution - increases from hardship, affects stamina drain
var constitution: float = 0.0
```

**Notes:**
- Constitution persists across days (global var, not level1)
- Used elsewhere for stamina calculations (future integration)

### 3. Red Vignette Flash
...
### 4. Overseer Warning Dialog Pools
...
const CONSTITUTION_PER_WHIP: float = 0.5
...
func _check_rage_thresholds():
    ...
    if rage >= WHIP_THRESHOLD:
        var whips_due = 1 + (rage - WHIP_THRESHOLD) / WHIP_INTERVAL
...
func _apply_whip():
    ...
    # Increase constitution (subtle benefit)
    Global.constitution += CONSTITUTION_PER_WHIP
...
### 6. Integration with Coal Tracking
...
**Modify coal delivery handler (in furnace.gd or delivery system):**
```gdscript
func _on_coal_delivered():
...
func _on_overseer_warning(message: String):
    # Display as notification (yellow/orange styling)
    NotificationSystem.show_warning(message)

func _on_overseer_severe_warning(message: String):
    # Display as urgent notification (red styling)
    NotificationSystem.show_severe_warning(message)

func _on_player_whipped(stamina_removed: int):
    # Display whip notification + visual/audio feedback
    NotificationSystem.show_punishment("The overseer's whip bites deep. -%d stamina" % stamina_removed)
...
- `global.gd` - Add constitution variable
- `level1/coal_piece.gd` - Call RageManager.on_coal_dropped() and flash vignette
...
- `CONSTITUTION_PER_WHIP` (0.5) - Constitution gain per punishment
...
**Decision: Whip gives constitution**
...
- [ ] Global constitution variable added
- [ ] VignetteOverlay scene created
- [ ] Vignette flash_red() function working
- [ ] RageManager autoload created
- [ ] Warning dialog pool implemented
- [ ] Severe warning dialog pool implemented
- [ ] Whipping logic implemented
- [ ] Coal drop triggers rage increase
- [ ] Coal delivery triggers rage decrease check
- [ ] Notifications connected to rage signals
- [ ] Day reset calls reset_for_new_day()
- [ ] All tests passing
