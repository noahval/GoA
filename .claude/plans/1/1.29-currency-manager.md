...
- 1.x-currency-display.md (Global.refresh_all_currency_displays() helper)
...
enum CurrencyType {
	...
	GOLD = 2,
	PLATINUM = 3,
	HOLES = 4,    # Underworld currency (gambling)
	WEEPS = 5,    # Underworld currency (Frayed End)
}

# ===== CONVERSION RATES (to copper value) =====
# Official: Base 1000:1 ratios between tiers
...
const CONVERSION_RATES = {
	...
	CurrencyType.HOLES: 0.02,      # 50 holes = 1 copper (terrible rate)
	CurrencyType.WEEPS: 0.01,      # 100 weeps = 1 copper (even worse rate)
}
...
# Note: Underworld currencies (holes, weeps) don't have market modifiers initially
var conversion_rate_modifiers = {
	...
	CurrencyType.HOLES: 1.0,     # May add underground market volatility later
	CurrencyType.WEEPS: 1.0,     # May add underground market volatility later
}
...
const CURRENCY_NAMES = {
	...
	CurrencyType.HOLES: "Holes",
	CurrencyType.WEEPS: "Weeps",
}
...
const CURRENCY_NAMES_PLURAL = {
	...
	CurrencyType.HOLES: "Holes",       # Punch confetti from work cards
	CurrencyType.WEEPS: "Weep knots",  # Lethe Weep Knots
}
...
# Includes all currencies: official (copper/silver/gold/platinum) and underworld (holes/weeps)
func get_total_coin_count() -> int:
	...
	total += int(Level1Vars.currency.holes)
	total += int(Level1Vars.currency.weeps)
	return total
...
func _get_player_currency(currency_type: CurrencyType) -> float:
	match currency_type:
		...
		CurrencyType.HOLES:
			return Level1Vars.currency.holes
		CurrencyType.WEEPS:
			return Level1Vars.currency.weeps
		_:
			return 0.0
...
func _currency_type_to_string(currency_type: CurrencyType) -> String:
	match currency_type:
		...
		CurrencyType.HOLES: return "holes"
		CurrencyType.WEEPS: return "weeps"
		_: return ""
```
...
### 9. Currency Display Integration

**NOTE**: The `Global.refresh_all_currency_displays()` helper is implemented in 1.x-currency-display.md

**Usage pattern for exchanges**:
```gdscript
# After currency change in exchange
var result = CurrencyManager.exchange_currency(COPPER, SILVER, 100)
if result.success:
...
**Integration**: The exchange_currency() function should call Global.refresh_all_currency_displays() automatically after successful transactions. See 1.x-currency-display.md for the refresh helper implementation.
...
**game/v0.1/autoloads/global.gd** (modified in 1.x-currency-display.md)
- Uses refresh_all_currency_displays() function from currency-display plan
...
```gdscript
Level1Vars.currency  # Read/write currency amounts (copper/silver/gold/platinum/holes/weeps)
Level1Vars.CURRENCY_TO_COPPER_RATE  # Exchange rates for all currencies
...
**Prerequisites**:
- [ ] 1.x-currency-display.md completed (Global.refresh_all_currency_displays() must exist)
...
**Plan Version**: 1.2 (Moved Global.refresh_all_currency_displays() to 1.x-currency-display.md)
