...
func _apply_layout(scene_root: Control) -> void:
    ...
    # Use float() to avoid narrowing conversion warnings (int -> float)
...
func _apply_font_scaling(node: Node) -> void:
    ...
    Respects existing font_size overrides (e.g., progress bar labels set to 14px).
    """
    var scaled_font = get_scaled_font_size()

    if node is Label or node is Button or node is RichTextLabel:
        # Only apply if node doesn't have a custom theme font size already set
        # This allows manual overrides for special cases (titles, debug text, progress bar labels, etc.)
