...
Icons support both `.png` and `.jpg` extensions (tries png first, falls back to jpg).

| Currency | Icon Base Path | Status |
|----------|----------------|--------|
| Copper | `res://level1/icons/copper` | Exists (.png) |
| Silver | `res://level1/icons/silver` | Add later (when silver unlocks) |
| Gold | `res://level1/icons/gold` | Add later (when gold unlocks) |
| Platinum | `res://level1/icons/platinum` | Add later (when platinum unlocks) |
| Holes | `res://level1/icons/holes` | Underworld currency (gambling) |
| Weeps | `res://level1/icons/weeps` | Underworld currency (Frayed End) |
...
**Purpose**: Display all 3 currencies (copper, holes, weeps) in a single horizontal line.

**Use Case**: Scenes that need to show the player's full wallet (e.g., gambling scene).

**Implementation**: Wrapper component that instantiates 3 CurrencyDisplay instances.

### Component Structure

```
MultiCurrencyDisplay (PanelContainer) - Grey background panel
└── HBoxContainer - Internal container (created in code)
    ├── CopperDisplay (CurrencyDisplay) - currency_type = "copper"
    ├── HolesDisplay (CurrencyDisplay) - currency_type = "holes"
    └── WeepsDisplay (CurrencyDisplay) - currency_type = "weeps"
```

### Script: `res://ui/multi_currency_display.gd`
...
### Scene File: `res://ui/multi_currency_display.tscn`

**Scene Setup** (in Godot editor):
1. Create new scene with root node: PanelContainer
2. Rename root to "MultiCurrencyDisplay"
3. Attach script: `res://ui/multi_currency_display.gd`
4. Save as: `res://ui/multi_currency_display.tscn`

**Note**: The script dynamically creates the HBoxContainer and 3 CurrencyDisplay children in _ready(), so the scene file only contains the empty PanelContainer root.
...
func add_currency_display():
	...
	if menu:
		...
		menu.move_child(multi_display, 0)
```

### Design Decisions

**Decision**: Always show all 3 currencies
- **Rationale**: Player requested all 3 visible at once, even if 0
- **Benefit**: Player always sees their full wallet status
- **Note**: This overrides the is_currency_unlocked() behavior from CurrencyDisplay

**Decision**: Reuse CurrencyDisplay component
- **Rationale**: DRY principle, all formatting/refresh logic already implemented
- **Benefit**: Minimal new code, consistent styling across single and multi displays

**Decision**: 32px spacing between currencies
- **Rationale**: Clear visual separation of the 3 distinct currency types
- **Benefit**: Easy to distinguish the 3 currency amounts at a glance

**Decision**: Grey background panel (MutedButton style)
- **Rationale**: Visual grouping and subtle emphasis like settings button
- **Implementation**: PanelContainer with dark grey background (0.25, 0.25, 0.25, 0.3)
- **Benefit**: Currencies appear as a cohesive unit, stands out from menu without being distracting

**Decision**: Centered alignment
- **Rationale**: Balanced presentation in menu layout
- **Implementation**: HBoxContainer.ALIGNMENT_CENTER
- **Benefit**: Professional appearance, doesn't favor left or right edge
