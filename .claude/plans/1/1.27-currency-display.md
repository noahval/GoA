...
- 1.x-currency-manager.md (Global.refresh_all_currency_displays() helper)
...
- **Locked currencies greyed**: Icons greyed out when currency type not yet unlocked
...
func _format_number(value: float) -> String:
	...
	# TODO: Check if Godot has built-in thousand separator formatting
	# If "%,d" % int_value works, use that instead of custom implementation
...
func is_currency_unlocked() -> bool:
	# TODO: Integrate with currency unlock system when available
	# For now, only copper is unlocked
	return currency_type == "copper"
```

**Key Features**:
- @export properties for easy configuration in editor
- Auto-loading of currency icon based on type
- Thousand separator formatting (1234 → "1,234")
- Optional zero-hiding (useful for unlockable currencies)
- Group-based refresh via Global.refresh_all_currency_displays()
- Currency unlock detection (greyed icons for locked currencies)
...
- Separation: 8px (space between icon and text)
...
### 3. Integration with Global Helper

**Refresh Pattern**: Uses Global.refresh_all_currency_displays() from 1.x-currency-manager.md

**How it works**:
1. CurrencyDisplay adds itself to "currency_displays" group in _ready()
2. Global.refresh_all_currency_displays() calls refresh() on all group members
3. Currency transactions call Global.refresh_all_currency_displays() after changes

**In Global.gd** (implemented in 1.x-currency-manager.md):
```gdscript
func refresh_all_currency_displays():
	get_tree().call_group("currency_displays", "refresh")
```

**Usage after currency transaction**:
```gdscript
# After adding/removing currency
Level1Vars.currency.copper += 100
Global.refresh_all_currency_displays()  # All displays update
```
...
- Font size: Amount label uses FONT_NORMAL from ResponsiveLayout (auto-scaled)
- Icon size: 32x32px base, scales with resolution multiplier
- Spacing: 8px separation, scales proportionally
...
func _ready():
	...
	# Apply responsive scaling to icon
	var scale_factor = ResponsiveLayout.get_scale_factor()
	icon.custom_minimum_size = Vector2(32, 32) * scale_factor
```

**Note**: ResponsiveLayout.apply_to_scene() should handle font scaling automatically when applied to parent scene.
...
func test_format_number_handles_negatives():
	var result = display._format_number(-1234)
	assert_true(result.begins_with("-"), "Negative should have minus sign")
	assert_true(result.contains(","), "Negative should still have separator")
...
**Decision 7**: Locked currencies greyed out
- **Implementation**: is_currency_unlocked() method returns false for locked currencies
- **Visual**: Icons rendered with grey modulate when locked
- **Benefit**: Visual indication of progression without explicit tutorials
...
- [ ] _load_icon() function implemented
- [ ] _get_currency_amount() function implemented
- [ ] _format_number() function with thousand separators
- [ ] refresh() function implemented
- [ ] is_currency_unlocked() function implemented
- [ ] set_currency_type() function implemented
...
- [ ] Copper icon created at `res://level1/icons/copper.png`

### Integration
- [ ] Global.refresh_all_currency_displays() implemented (see 1.x-currency-manager.md)
...
**Plan Version**: 2.0 (Group-based refresh, TDD methodology, no static mode)
