...
var shovel_follow_speed: float = 10.0    # Velocity multiplier for mouse tracking
var shovel_linear_damp: float = 15.0     # How fast shovel stops moving
var shovel_mass: float = 100.0           # Shovel weight/inertia
var shovel_tilt_torque: float = 108000.0 # How fast shovel rotates when tilting
var shovel_max_rotation_degrees: float = 45.0  # Maximum tilt angle
...
# NOTE: In later phases, technique selection tracking is added here:
# - selected_techniques: Dictionary (per-run technique picks, resets each day)
# - upgrades_qty: int (total upgrades this run)
# - clean_streak_unlocked: bool (derived from techniques)
# - heavy_combo_unlocked: bool (derived from techniques)
```
...
func load_save_data(data: Dictionary):
    ...
    # Physics upgrades (with defaults matching initial values)
...
- **Max stamina**: 100 - Can be increased through upgrades in later plans
- **Starting focus**: 50 - Enough for decision-making and mental tasks
- **Max focus**: 100 - Can be increased through upgrades in later plans

### Coal Tracking Stats

- **coal_spilled**: 0 - Counts coal pieces that fell off shovel or were dropped
- **coal_shovelled**: 0 - Counts coal pieces successfully delivered to furnace
- These are cumulative stats tracked across gameplay sessions
- Used for progression metrics, achievements, and efficiency calculations
