...
**Decision**: Combined save approach with save_data dictionary
- **Rationale**: Simpler than separate settings + game save files
...
- **Cloud compatibility**: This save_data structure and surgical preservation logic applies to both local saves (FileAccess) and cloud saves (Nakama). When using cloud saves, the save()/load_save() functions are replaced with NakamaClient.save_game()/load_game() calls (see 1.x-nakama-server.md), but the data structure and settings preservation remain the same.
