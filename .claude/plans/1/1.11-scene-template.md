...
- **Menu**: Right side menu for stats/buttons (sized by ResponsiveLayout)
  - **SettingsButton**: Muted button present in all scenes (uses MutedButton theme variation - dark grey, 30% opacity)
...
- Add SettingsButton to Menu with text "Settings"
- Set SettingsButton's `theme_type_variation` to `&"MutedButton"` (dark grey, 30% opacity - low-prominence utility style)
