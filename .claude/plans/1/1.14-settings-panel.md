...
func _on_dev_speed_toggled(pressed: bool):
	...
	# Note: dev_speed_mode is session-only, not saved (always starts false on launch)
...
# Session-only setting (not persisted - always starts false on launch)
...
- [ ] State persists during session but resets to off on game restart (not saved)

**Save Reset:**
- [ ] Button shows confirmation dialog
- [ ] Confirming resets game progress
- [ ] Settings preserved after reset (UI scale, volumes)
