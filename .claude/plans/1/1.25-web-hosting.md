...
2. **CRITICAL: Verify your email address**
   - Check inbox for verification email from itch.io
   - Click verification link
   - **This MUST be done before first deployment** - Butler will reject uploads otherwise
   - Error message if skipped: "Please verify your account's email address before uploading a build"
  ...
  export-and-deploy:
    ...
    steps:
      ...
      - name: Deploy to itch.io
        ...
        run: |
          ./butler push build/web ${{ secrets.ITCH_USER }}/${{ secrets.ITCH_GAME }}:html5 --userversion ${{ github.sha }}
```

**Why these choices:**
- Direct Butler CLI installation from `broth.itch.zone` (official itch.io CDN)
- Third-party GitHub Actions have reliability issues - direct CLI is simpler
- `BUTLER_API_KEY` environment variable for authentication
- `html5` channel for web builds
- `--userversion ${{ github.sha }}` for git commit hash versioning
...
- Email must be verified on itch.io account (one-time requirement)
...
**Decision 2:** Use Butler CLI directly (not via GitHub Action)
- **Rationale**:
  - Official itch.io deployment tool
  - Direct installation from broth.itch.zone is reliable
  - Third-party GitHub Actions had reliability issues (Docker build failures)
  - Direct CLI approach is simpler and more transparent
- **Alternatives considered**:
  - Third-party GitHub Actions (rejected - unreliable Docker builds)
...
- [x] Verify itch.io email address (required before first deploy)
...
- [x] Test game on itch.io project page
...
**Causes:**
- Email not verified on itch.io account
- Invalid API key
- itch.io project doesn't exist
- Username or game slug wrong

**Fix:**
1. Check for "verify your account's email" error - if so, verify email through itch.io link
2. Verify ITCH_API_KEY is correct in GitHub secrets
3. Check itch.io project exists and is HTML5 type
4. Verify ITCH_USER matches your exact username
5. Verify ITCH_GAME matches your project slug exactly
6. Check itch.io API key hasn't been revoked
